...
public class Subsets
{
	...
	public int findSubset(int[] numbers)
	{
	    ...
	    Arrays.sort(numbers);
	    numbers = removeArray(numbers, 0);
	    int[] ar = {1};
	    dfs(1, 1, numbers, ar);
		return count;
	}

    public void dfs(int bagSize, int sum, int[] numbers, int[] used) {
        if (bagSize > sum) {
            count ++;
//            System.out.println(Arrays.toString(used));
        ...
        while (numbers.length > 0) {
            ...
            bef = now;
            int nextBag = bagSize + now;
            int next = sum * now;
            if (next >= nextBag) {
                break;
            }
            int[] nextU = addArray(used, now);
            dfs(nextBag, next,  numbers, nextU);
        }
    }
